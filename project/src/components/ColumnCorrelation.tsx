...
const ColumnCorrelation = () => {
  ...
  // AI Summary Functions - Simplified
  ...
  return (
    <div className="p-4 bg-gradient-to-br from-blue-50 to-indigo-50">
      ...
      <div className="mb-3 flex justify-between items-center bg-white rounded-lg p-2 shadow-md border border-gray-200">
        <div className="flex gap-2">
          <button 
            ...
            className={`px-3 py-2 rounded-md border ${view === 'global' ? 'bg-blue-600 text-white border-blue-700' : 'bg-gray-100 border-gray-300'}`}
          >
            Global
          </button>
          <button 
            onClick={() => handleViewChange('custom')}
            className={`px-3 py-2 rounded-md border ${view === 'custom' ? 'bg-blue-600 text-white border-blue-700' : 'bg-gray-100 border-gray-300'}`}
          >
            Custom
          </button>
        </div>
      ...
      <div className="flex flex-col lg:flex-row gap-3 h-full">
        {/* Controls Panel - Condensed and only shown for custom view */}
        ...
        {!isLoading && matrix.length > 0 && (
          <div className={`${view === 'custom' ? 'lg:w-3/4' : 'w-full'}`}>
            <div ref={matrixRef} className="bg-white rounded-lg shadow-md p-3 relative overflow-hidden h-full border border-gray-200">
              ...
              {hoverData && (
                <div 
                  className="absolute z-10 rounded-md shadow-md bg-blue-600 text-white p-2 text-xs border border-blue-800"
                  style={{ 
                    left: `${hoverData.x}px`, 
                    top: `${hoverData.y}px`,
                ...
                >
                  <div>{hoverData.col1} × {hoverData.col2}</div>
                  <div className="font-bold">{hoverData.value.toFixed(2)}</div>
                  <div>{getStrength(hoverData.value)}</div>
                </div>
              )}
              
              {/* Mini Legend */}
              <div className="mb-2 flex justify-end gap-1 items-center text-xs border-b border-gray-200 pb-1">
                <div className="flex items-center">
                  <div className="w-4 h-4 bg-white border border-gray-300"></div>
                  <span className="ml-1">0</span>
                </div>
                <div className="flex items-center">
                  <div className="w-4 h-4 bg-blue-300 border border-blue-400"></div>
                  <span className="ml-1">0.3</span>
                </div>
                <div className="flex items-center">
                  <div className="w-4 h-4 bg-blue-600 text-white text-center border border-blue-700"></div>
                  <span className="ml-1">0.7</span>
                </div>
                <div className="flex items-center">
                  <div className="w-4 h-4 bg-blue-900 text-white text-center border border-blue-950"></div>
                  <span className="ml-1">1</span>
                </div>
              </div>
              
              {/* Compact Matrix Table with Borders */}
              <div className="overflow-auto" style={{maxHeight: "calc(100vh - 200px)"}}>
                <table className="text-xs w-full table-fixed border-collapse">
                  <thead>
                    <tr>
                      <th className="p-1 w-12 border border-gray-300 bg-gray-50"></th>
                      {uniqueColumns.map((col, idx) => (
                        <th key={idx} className="p-1 w-10 overflow-hidden border border-gray-300 bg-gray-50">
                          <div className="transform -rotate-45 origin-left truncate w-14">
                            {getSimpleName(col)}
                          </div>
                        </th>
                      ))}
                  ...
                  <tbody>
                    {matrix.map((row, i) => (
                      <tr key={i}>
                        <td className="p-1 font-medium truncate border border-gray-300 bg-gray-50">{getSimpleName(uniqueColumns[i])}</td>
                        {row.map((value, j) => (
                          <td
                            key={j}
                            className="p-1 w-8 h-8 text-center hover:scale-105 border border-gray-300"
